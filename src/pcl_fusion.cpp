...
pcl::PointCloud<pcl::PointXYZI>::Ptr range_filter(pcl::PointCloud<pcl::PointXYZ>::Ptr& pc, double inlier_r_min, double inlier_r_max)
{
...
//Callback
void callback(const sensor_msgs::PointCloud2ConstPtr& msg_pc1, const sensor_msgs::PointCloud2ConstPtr& msg_pc2, const sensor_msgs::PointCloud2ConstPtr& msg_pc3)
{
    pcl::PointCloud<pcl::PointXYZ>::Ptr sensor1_cloud (new pcl::PointCloud<pcl::PointXYZ>),
                                        ...
                                        sensor3_cloud (new pcl::PointCloud<pcl::PointXYZ>),
                                        fused_cloud (new pcl::PointCloud<pcl::PointXYZ>);
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr temp_cloud1 (new pcl::PointCloud<pcl::PointXYZI>),
                                        ...
                                        temp_cloud3 (new pcl::PointCloud<pcl::PointXYZI>);
    ...
    temp_cloud3 = range_filter(sensor3_cloud, 0.0, range_3);
    pcl::copyPointCloud(*temp_cloud3, *sensor3_cloud);
    ...
    sensor_msgs::PointCloud2 cloud3_ros;
    pcl::toROSMsg(*sensor3_cloud, cloud3_ros);
    cloud3_ros.header = msg_pc1->header;
    cloud3_pub_.publish(cloud3_ros);

    ROS_WARN("<<<<<<<<<<<<<<< Listen TF <<<<<<<<<<<<<<<");
    tf::TransformListener listener1;
    tf::StampedTransform transform1;
    try{
        cout << "listen for tf1: " << endl;
        listener1.waitForTransform("sensor1_gt", "world_gt", ros::Time(0), ros::Duration(2));
        listener1.lookupTransform ("sensor1_gt", "world_gt", ros::Time(0), transform1);
    ...
    try{
        cout << "listen for tf2: " << endl;
        listener2.waitForTransform("sensor2_gt", "world_gt", ros::Time(0), ros::Duration(2));
        listener2.lookupTransform ("sensor2_gt", "world_gt", ros::Time(0), transform2);
    }catch (tf::TransformException& ex) {
        ROS_WARN("TF exception:\n%s", ex.what());
        return;
    }
    ...
    try{
        cout << "listen for tf3: " << endl;
        listener3.waitForTransform("sensor3_gt", "world_gt", ros::Time(0), ros::Duration(2));
        listener3.lookupTransform ("sensor3_gt", "world_gt", ros::Time(0), transform3);
    }catch (tf::TransformException& ex) {
        ROS_WARN("TF exception:\n%s", ex.what());
        return;
    }
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud1_world (new pcl::PointCloud<pcl::PointXYZ>),
                                        cloud2_world (new pcl::PointCloud<pcl::PointXYZ>),
                                        cloud3_world (new pcl::PointCloud<pcl::PointXYZ>);
    pcl_ros::transformPointCloud(*sensor1_cloud, *cloud1_world, transform1.inverse());
    pcl_ros::transformPointCloud(*sensor2_cloud, *cloud2_world, transform2.inverse());
    pcl_ros::transformPointCloud(*sensor3_cloud, *cloud3_world, transform3.inverse());
...
int main(int argc, char **argv)
{
    ...
    && ros::param::get("~cloud3_tp", cloud3_tp)
    ...
    Synchronizer<MySyncPolicy> sync(MySyncPolicy(10),cloud1_sub, cloud2_sub, cloud3_sub);
