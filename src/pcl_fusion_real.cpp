...
void callback_rectify1(const sensor_msgs::PointCloud2ConstPtr& msg_pc)
{
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_in (new pcl::PointCloud<pcl::PointXYZI>),
                                        cloud_world (new pcl::PointCloud<pcl::PointXYZI>);
...
void callback_register2(const sensor_msgs::PointCloud2ConstPtr& msg_pc1, const sensor_msgs::PointCloud2ConstPtr& msg_pc_ref)
{
    ...
    *fused_cloud_rgb += *sensor2_cloud_rgb;
    ...
    cloud1_world_ros.header.frame_id = frame_id_;
    ...
    cloud2_world_ros.header.frame_id = frame_id_;
    ...
    fused_cloud_ros.header.frame_id = frame_id_;
    ...
    fused_cloud_rgb_ros.header.frame_id = frame_id_;
...
void loadParam()
{
    ...
    if(ros::param::get("~exParamFile_dir_", exParamFile_dir_))
    {
        ...
        if(ros::param::get("~use_inverse_tr_", use_inverse_tr_))
            ROS_INFO("Retrived param 'use_inverse_tr_': %d", use_inverse_tr_);        
        ostringstream os_in;
        os_in << exParamFile_dir_;
        Eigen::Matrix4d Tr_ = load_exParam(os_in.str().c_str());
        if(use_inverse_tr_)
            Tr = Tr_.inverse();
        else
            Tr = Tr_;
        // Tr = load_exParam(os_in.str().c_str());
        // cout << "tr_.inverse = " << endl << Tr_.inverse() << endl;
